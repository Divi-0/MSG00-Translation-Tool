...
{
    public abstract class CsvbReader
    {
        ...
        private static async Task<CsvbFileType> ReadCsvbFileTypeAsync(Stream fileStream)
        {
            List<byte> data = new List<byte>();
            var currentBytes = new byte[4];

            fileStream.Seek(0x20, SeekOrigin.Begin);
            await fileStream.ReadExactlyAsync(currentBytes, 0x00, 4).ConfigureAwait(false);
        ...
        /// <param name="fileStream"></param>
        /// <returns></returns>
        private static async Task<CsvbHeader> ReadFileHeaderAsync(Stream fileStream)
        {
            fileStream.Seek(0, SeekOrigin.Begin);
            var staticFirstHeaderBytes = new byte[0x0C];
            await fileStream.ReadExactlyAsync(staticFirstHeaderBytes, 0x00, 12).ConfigureAwait(false);

            fileStream.Seek(0, SeekOrigin.Begin);
            var fileOffsetToAreaBetweenPointerAndTextTableBytes = new byte[4];
            await fileStream.ReadExactlyAsync(staticFirstHeaderBytes, 0x0C, 4).ConfigureAwait(false);

            fileStream.Seek(0, SeekOrigin.Begin);
            var fileOffsetToTextTableBytes = new byte[4];
            await fileStream.ReadExactlyAsync(staticFirstHeaderBytes, 0x10, 4).ConfigureAwait(false);

            fileStream.Seek(0, SeekOrigin.Begin);
            var fileOffsetToAreaBetweenTextAndOffsetTableBytes = new byte[4];
            await fileStream.ReadExactlyAsync(staticFirstHeaderBytes, 0x14, 4).ConfigureAwait(false);

            fileStream.Seek(0, SeekOrigin.Begin);
            var fileOffsetToOffsetTableBytes = new byte[4];
            await fileStream.ReadExactlyAsync(staticFirstHeaderBytes, 0x18, 4).ConfigureAwait(false);
        ...
        public virtual async Task<CsvbFile> ReadAsync(Stream stream)
        {
            var header = await ReadFileHeaderAsync(stream);
            var csvbType = await ReadCsvbFileTypeAsync(stream);
